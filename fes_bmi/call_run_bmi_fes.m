...
params.output           = 'catch'; % 'stimulator'; 'wireless_stim';
...
if ~isstruct(dec_file)
    ...
    if isfield(N2E, 'H')
        ...
        for i = 1:length(emg_pos_in_dec)
            emg_pos_in_dec(1,i) = find( strncmp(N2E.outnames,emg_list_4_dec(i),length(emg_list_4_dec{i})) );
        end
...
% for an 'offline' experiment, load the file we will be replaying
if ~params.online
    if ismac
...
% also, by default, always save the data in an online experiment
if params.online
    params.save_data    = true;
end
...
params.display_plots    = true; % these plots are for other BMI stuff
...
sp.EMG_min              = [0.0 0.0 0.0]; %repmat( 0.2, 1, numel(sp.muscles));
sp.EMG_max              = [0.8 0.8 0.8]; %repmat( 1, 1, numel(sp.muscles));
