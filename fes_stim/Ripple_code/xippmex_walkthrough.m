...
%% Filter Settings
% Set the Hi-Res notch filter for channels 9-12 on the Micro+Stim to use
% the 60/120/180 comb filter
if ~isempty(stimChans)
    [selection, filters] = xippmex('filter','list',stimChans(9:12),'hires notch');
    filterNum = find(strcmp({filters.label}, '60/120/180 Hz'));
    xippmex('filter','set', stimChans(9:12),'hires notch', filterNum);
end

% Set the LFP filter for recording channels 1-8 to use wide-band 0.3-250 Hz filter
if ~isempty(recChans)
    [selection, filters] = xippmex('filter','list',recChans(1:8),'lfp');
    filterNum = find(strcmp({filters.label}, '0.3-250 Hz'));
    xippmex('filter','set', stimChans(9:12),'lfp', filterNum);
end
...
% a train length of tw0 seconds. In this case, electrode 5 will have the 
% cathodic phase first and electrode 10 will have the anodic phase first. 
% So for the first phase, current will flow from electrode 10 into 
% electrode 9.
if ~isempty(stimChans)    
     stimString = sprintf('Elect=%d,%d,;TL=2000.0,2000.0,;Freq=30,30,;Dur=0.4,0.4,;Amp=20,20,;TD=0.0,0.0,;FS=1.0,1.0,;PL=1,0,;',stimChans(5),stimChans(6));
