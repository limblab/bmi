...
if ttap.save_data_yn
    ...
    hw.cb.full_file_name        = fullfile( hw.data_dir, [ttap.monkey '_' ttap.bank '_' num2str(ttap.stim_elec) '_' hw.start_t '_' ttap.task '_TTA' ]);
...
if ttap.record_emg_yn
    ...
    % emg.stimulated_channels has info on which channels [stim time, stim electrode, boolean on if cbmex found sync pulse]
    ...
    emg.stimulated_channels = zeros(ttap.nbr_stims_ch*numel(ttap.stim_elec),3);
else
    emg = [];
end
...
if ttap.record_force_yn
    ...
    % force.stimulated_channels has info on which channels were stimulated  [stim time, stim electrode, boolean on if cbmex found sync pulse]
    ...
    force.stimulated_channels = zeros(ttap.nbr_stims_ch*numel(ttap.stim_elec),3);
else
    force = [];
end
...
total_stim_count = 0; % counter for how many total successful stims

while hw.curr_stim_nbr <= ttap.nbr_stims_ch && ishandle(hw.keep_running)
    disp(hw.curr_stim_nbr)
    ...
    for i = 1:hw.nbr_elecs
        ...
        elseif strncmp(ttap.control_mode,'word',4)
            cbmex('trialconfig', 1);
            words = [];
            disp(['Waiting for word ' num2str(ttap.word) '...'])
            while isempty(words) && ishandle(hw.keep_running)
                % Read the words from central
        ...
        [ts_cell_array, cb_system_time, analog_data] = cbmex('trialdata',1);
        cbmex('trialconfig', 0);
        drawnow;
        ...
        % Store time of sync pulse and stimulated channel into matlab
        % structure for use later
        total_stim_count = total_stim_count + 1;
        if ttap.record_force_yn
            force.stimulated_channels(total_stim_count,:) = [cb_system_time, hw.elec_order(i) sync_pulse_flag];
        end
        if ttap.record_emg_yn
            emg.stimulated_channels(total_stim_count,:) = [cb_system_time, hw.elec_order(i) sync_pulse_flag];
        end
...
if ttap.save_data_yn
    ...
    elseif ~ttap.record_emg_yn
        save(hw.matlab_full_file_name,'force','ttap');
        disp(['Force data and Stim Params saved in ' hw.matlab_full_file_name]);
    else
