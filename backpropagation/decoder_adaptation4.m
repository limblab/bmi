function [data_buffer,data,neuron_decoder] = decoder_adaptation4(params,data,bin_count,data_buffer,neuron_decoder,emg_decoder,force_pred)
...
if data.adapt_flag
    ...
    if ~data.fix_decoder % && any(max(data_buffer.tgt_id{1}) == [1 5])
        ...
        for trial = 1:params.batch_length
            ...
            init_emg_val   = zeros(num_tgts,size(emgs,2));
            
            for t = 1:num_tgts
                unique_tgt_pos(t,:) = tgt_pos(find(tgt_id==unique_tgt_ids(t),1,'last'),:);
                init_emg_val(t,:)   = emgs(find(tgt_id==unique_tgt_ids(t),1,'last'),:);
...
%                                         'TolX',TolX,'TolFun',TolFun,'TypicalX',TypicalX);
            ...
            [unique_opt_emgs,~,exitflag] = fmincon(@(EMG) Force2EMG_costfun_sig(EMG,unique_tgt_pos,emg_decoder,params.lambda),init_emg_val,[],[],[],[],emg_min,emg_max,[],fmin_options);
            if ~exitflag
            ...
            for t= 1:num_tgts
                ...
                opt_emgs(tgt_idx,:) = repmat(unique_opt_emgs(t,:),sum(tgt_idx),1);
            end
